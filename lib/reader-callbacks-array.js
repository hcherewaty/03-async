...
const readAll = (paths, callback) => {
  ...
  let useReadTheFile = (cb) => {
    console.log('Part I: Calling readTheFile');
    let info = contents.push(data.toString().trim());
    cb(undefined, info);
    console.log('Part III: After readTheFile runs');
  }; 

  for (let i in paths) {
    useReadTheFile(readTheFile(paths[i]));
  }
};
